...
def run_bma_model(X_train, X_test, Y_train,
  ...
  model_config['activation_func']="softmax"
  model_config['activation']='cosine'
...
def bma_dist(inputs: tf.Tensor, 
  ...
  if posterior_mode and sample_intermediate_variables:
    ...
    joint_dist["ensemble_weights"] = lambda gps: tfd.Deterministic(
        loc=build_ensemble_weights(gps, activation_func))
...
# compute the test negative loss likelihood: mean( (y_true - y_pred)^2 / \sigma_var). where sigma_pred is the predictive standard error for each observation. 
def nll(y_true, y_pred):
  # y_true: true value of y
  # y_pred: predicted value of y
  # sigma_pred: predictive standard error for each observation
