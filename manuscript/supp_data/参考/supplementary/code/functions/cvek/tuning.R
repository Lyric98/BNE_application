...
#' \Big\}}
#' 
#' @param Y (vector of length n) Responses of the dataframe.
...
tuning <-
  function(Y, fixed_X, K_mat, mode, lambda) {
    
    mode <- match.arg(mode, c("AIC", "BIC", "GCV", "AICc", "GCVc", "gmpml", "loocv"))
    func_name <- paste0("tuning_", mode)
    lambda_selected <- do.call(func_name, list(Y = Y, fixed_X = fixed_X, 
                                               K_mat = K_mat, lambda = lambda))
...
tuning_AIC <-
  function(Y, fixed_X, K_mat, lambda) {
    ...
    CV <- sapply(lambda, function(k) {
      ...
      proj_matrix <- 
        estimate_ridge(X = as.matrix(cbind(1, fixed_X)),
