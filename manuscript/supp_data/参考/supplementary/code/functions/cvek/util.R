...
generate_data <-
  function(n, label_names, 
           ...
           l = 1, p = 2, eps = .01, dis_type = 0) {
    
    fixed_X <- rmvnorm(n = n, mean = rep(0, 5), sigma = diag(5))
    beta <- rnorm(5)
    XB <- fixed_X %*% beta
    XB <- XB / sqrt(sum(XB ^ 2))
    ...
    if (length(Ks$d / sum(Ks$d) > .001) > 0) {
      ...
      if (all(h1 == 0)) {
        warning("interaction term colinear with linear space!")
        h1 <- h1_prime
        h1 <- h1 / sqrt(sum(h1 ^ 2))
      } else {
        h1 <- h1 / sqrt(sum(h1 ^ 2))
    ...
    Y <- XB + h0 + int_effect * h1 + resi
    data <- as.data.frame(cbind(Y, fixed_X, X1, X2))
    fixed_name <- paste0("r", seq(5))
    colnames(data) <- c("Y", fixed_name, label_names[[1]], label_names[[2]])
...
estimate_noise <- function(Y, fixed_X, lambda_hat, beta_hat, alpha_hat, K_hat) {

  n <- nrow(K_hat)
  V_inv <- ginv(K_hat + lambda_hat * diag(n))
  oneX <- as.matrix(cbind(1, fixed_X))
  Px <- oneX %*% ginv(t(oneX) %*% V_inv %*% oneX) %*% t(oneX) %*% V_inv
  Pk <- K_hat %*% V_inv %*% (diag(n) - Px)
  A <- Px + Pk
  SSE <- sum((Y - oneX %*% beta_hat - K_hat %*% alpha_hat) ^ 2)
  sigma2_hat <- SSE / (n - sum(diag(A)) - 1)
...
#' @export compute_stat
compute_stat <-
  function(n, Y, K12, fixed_effect, K_gpr, sigma2_hat, tau_hat) {

    K0 <- K_gpr
    
    V0_inv <- ginv(tau_hat * K0 + sigma2_hat * diag(n))
    
    test_stat <- tau_hat * t(Y - fixed_effect) %*% V0_inv %*%
      K12 %*% V0_inv %*% (Y - fixed_effect) / 2
