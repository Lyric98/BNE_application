...
#' @export estimation
estimation <- function(
  Y, fixed_X, X1, X2, kern_list, mode = "loocv", strategy = "erm", 
  beta = 1, lambda_list = exp(seq(-10, 5))) {
  # base model estimate
  n <- length(Y)
  kern_size <- length(kern_list)
  base_est <- estimate_base(n, kern_size, Y, fixed_X, X1, X2, 
                            kern_list, mode, lambda_list)
  ...
  # final estimate
  lambda_ens <- tuning(Y, fixed_X, K_ens, mode, lambda_list)
  ens_est <- estimate_ridge(X = as.matrix(cbind(1, fixed_X)),
                            K = K_ens, Y = Y, lambda = lambda_ens)
  y_hat <- ens_est$proj_matrix$total %*% Y
...
estimate_base <- function(n, kern_size, Y, fixed_X, X1, X2, 
  ...
  for (d in seq(kern_size)) {
    ...
    if (length(lambda) != 0) {
      lambda0 <- tuning(Y, fixed_X, K, mode, lambda)
      estimate <- estimate_ridge(X = as.matrix(cbind(1, fixed_X)),
